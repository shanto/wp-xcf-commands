...
A WordPress plugin which adds a set of new search results to the site editor command palette (CMD+K/Ctrl+K) for all items filed under custom types except for those ignored from the settings page.

[![](./demo.gif)](https://sforge.neetorecord.com/embeds/xcf-commands?autoplay=1)
...
Using it is very easy. Just search for "XCF Commands" in WordPress plugin directory, or download a copy from the Github repo. Install, and activate. That's it! If necessary exclude types you want not to appear in the command palette.
